...
This repository is the **open idea plus a working board**. Self-host it. There is no public hosted account. Generic intro copy lives in `site/`.
...
- Snapshot API: agents POST a redacted board (Bearer token). Snapshot updates lists and device stamps. It does not overwrite the continue packet (`/api/context` owns that).
- The write token also covers `/api/now`, `/api/context`, and `/api/handoff` so the CLI/MCP works without a browser cookie.
- Cloudflare Pages + KV. One KV key: `board`.
- Paths in snapshots are slugs (`~/project`), never absolute home directories. Already-slugged values stay slugged. The server redacts again on write.
- Public intro site under `site/` (repo CTA only). Marketing copy stays generic: a website you host.
...
- Self-host. You own the store and the password.
